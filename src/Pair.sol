...
contract Pair is ERC20Upgradeable {
    ...
    modifier validAmount(
     ...
     function initialize(
        address _router,address _tokenA,address _tokenB,address _weth,string memory pairsymbol
    ...
    function swap(address _recipient,address tokenIn,address tokenOut,uint amountTokenIn,uint amountTokenOut) external onlyRouter  returns(uint) {
    ...
    function mint(address _recipient, uint _amountTokenA,uint _amountTokenB) external onlyRouter {
        uint sharemintAmount = liquidityMAth.share(_amountTokenA,_amountTokenB,tokenATotalBal ,totalShareInPool);
        totalShareInPool +=sharemintAmount;
        _mint(_recipient,sharemintAmount);
    } 

    function burn(address _recipient ,uint _sharemintAmount) external onlyRouter returns(uint,uint){
        require (balanceOf(_recipient) >= _sharemintAmount );
        (uint amountTokenA,uint amountTokenB) = liquidityMAth.removeLiquidityCalc(_sharemintAmount,totalShareInPool,tokenATotalBal,tokenBTotalBal);
    ...
    function updateReserve(uint _amountTokenA,uint _amountTokenB) external onlyRouter{
